...
$siswaCounts = [];
$res = $db->query("SELECT tempat_pkl_id, COUNT(*) as total FROM siswa GROUP BY tempat_pkl_id");
foreach ($res->fetchAll() as $row) $siswaCounts[$row['tempat_pkl_id']] = $row['total'];
...
<div class="card" style="margin-bottom:12px;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;">
        <div style="flex:1;">
            <div style="font-weight:700;font-size:0.95rem;"><?= $tp['nama'] ?></div>
            <div style="font-size:0.78rem;color:var(--text-muted);margin-top:3px;"><?= $tp['alamat'] ?></div>
            <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">
                <span style="font-size:0.7rem;background:var(--primary-light);color:var(--primary);padding:3px 10px;border-radius:20px;font-weight:600;">
                    <?= $siswaCounts[$tp['id']] ?? 0 ?> Siswa
                </span>
                <?php if ($tp['latitude'] && $tp['longitude']): ?>
                <span style="font-size:0.7rem;background:#d1fae5;color:#065f46;padding:3px 10px;border-radius:20px;font-weight:600;">
                    <i class="fas fa-location-dot"></i> GPS Tersetting
                </span>
                <?php else: ?>
                <span style="font-size:0.7rem;background:#fee2e2;color:#991b1b;padding:3px 10px;border-radius:20px;font-weight:600;">
                    Belum Ada GPS
                </span>
                <?php endif; ?>
            </div>
        ...
        <div style="display:flex;flex-direction:column;gap:6px;">
            ...
            <form method="POST" action="" onsubmit="return confirm('Hapus tempat ini?')">
                ...
                <button class="btn" style="padding:7px 12px;width:auto;font-size:0.8rem;background:var(--error);color:white;">
...
<div class="bottom-sheet" id="sheetTempat" style="max-height:92vh;overflow-y:auto;">
    ...
    <h3 id="sheetTitle" style="margin-bottom:15px;font-size:1.1rem;"><i class="fas fa-building" style="color:var(--primary);"></i> Tempat PKL</h3>
    ...
    <form method="POST" action="" id="formTempat">
        ...
        <div class="form-group">
            <label class="form-label">Lokasi GPS <small style="color:var(--text-muted)">(klik peta atau drag marker)</small></label>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;align-items:end;">
                <div>
                    ...
                    <input type="text" name="latitude" id="tLat" class="form-control" placeholder="-6.200000" style="padding:10px;">
                </div>
                <div>
                    <label class="form-label" style="font-size:0.75rem;">Longitude</label>
                    <input type="text" name="longitude" id="tLng" class="form-control" placeholder="106.816666" style="padding:10px;">
                </div>
                <div>
                    <label class="form-label" style="font-size:0.75rem;">Radius (m)</label>
                    <input type="number" name="radius_meter" id="tRadius" class="form-control" value="100" style="padding:10px;">
                </div>
